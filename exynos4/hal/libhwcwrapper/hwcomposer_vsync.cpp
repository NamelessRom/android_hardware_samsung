...
static void *hwc_vsync_thread(void *data)
{
    ...
    vsync_timestamp_fd = open(VSYNC_TIME_PATH, O_RDONLY);
    ...
    do {
        ...
        if (ctx->procs) {
            if (DEBUG_VSYNC) {
                ALOGD("%s: handling vsync, timestamp:%d", __FUNCTION__, timestamp);
            }
...
void init_vsync_thread(hwc_context_t* ctx)
{
    ...
    ret = pthread_create(&vsync_thread, NULL, hwc_vsync_thread, (void*) ctx);
    if (ret) {
