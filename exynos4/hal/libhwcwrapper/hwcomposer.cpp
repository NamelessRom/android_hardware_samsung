...
{
    if (!format_is_supported_by_s3c(format))
        return true;

    uint8_t bpp = format_hal_to_bpp(format);
...
/*    dst_crop_w_alignement = GSC_DST_CROP_W_ALIGNMENT_RGB888;
    ...
    bool rc = (dest_w % dst_crop_w_alignement) == 0;
...
bool supports_overlay(struct hwc_context_t *ctx, const hwc_layer_1_t &layer, size_t i)
{
    ...
    if (requires_gscaler(layer, handle->format)) {
        if (!supports_fimg(layer, handle->format)) {
            ALOGD("\tlayer %u: gscaler required but not supported", i);
            return false;
        }
    } else {
...
/*static inline bool fimg_dst_cfg_changed(exynos_gsc_img &c1, exynos_gsc_img &c2)
{
    return c1.x != c2.x ||
            c1.y != c2.y ||
            c1.w != c2.w ||
...
static inline bool fimg_src_cfg_changed(exynos_gsc_img &c1, exynos_gsc_img &c2)
{
    return fimg_dst_cfg_changed(c1, c2) ||
            c1.fw != c2.fw ||
            c1.fh != c2.fh;
}*/
