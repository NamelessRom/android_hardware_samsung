            ...
            if (layer.compositionType == HWC_OVERLAY) {
                ...
                if (requires_gscaler(layer, handle->format)) {
                    ...
                    if ((WIDTH(layer.displayFrame) != ctx->win[nextWindow].rect_info.w) ||
                        ...
                        //ALOGD("%s: window(%d) win(%d, %d) layer(%d, %d) need to reallocate", __FUNCTION__, nextWindow,
                        //        ctx->win[nextWindow].rect_info.w, ctx->win[nextWindow].rect_info.h, WIDTH(layer.displayFrame), HEIGHT(layer.displayFrame));
